...
describe("Contributors API", () => {
  ...
  test("List all contributors with illegal paginate query", async () => {
    // No paginate query.
    const mockRequest = ({
      query: {
        current: "invalid",
    ...
    expect(json.mock.calls[0][0].message).toBe(
      ContributorMessage.IllegalQueryParameters
    );
    // Assert status.
    expect(status.mock.calls.length).toBe(1);
    expect(status.mock.calls[0][0]).toBe(StatusCodes.BAD_REQUEST);
  });
});
